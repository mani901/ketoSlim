...
// App composition: wraps routes with global providers (dark mode, form)
// and an error boundary for graceful runtime failures.
const App: React.FC = () => {
  return (
    <DarkModeProvider>
      <FormProvider>
      <div className="app-container min-h-screen w-full flex flex-col items-center justify-center px-2 sm:px-4 py-6 transition-colors duration-200">
        ...
        {/* Routing (guarded by error boundary) */}
        <Router>
          <ErrorBoundary>
            <Routes>
              ...
              <Route path="*" element={<div className="text-heading-sales text-center">Page not found</div>} />
            </Routes>
          </ErrorBoundary>
        </Router>
      </div>
