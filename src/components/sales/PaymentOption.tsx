  ...
  return (
    <div
      ...
      className={`relative border-2 rounded-xl p-4 flex items-center justify-between cursor-pointer transition
        ${
          isSelected
            ? isDarkMode
              ? "border-[#36BC9F] bg-[#181a1b]"
              : "border-[#36BC9F] bg-white"
            : isDarkMode
    ...
    >
      <div className="flex-1 relative">
        ...
        {discount && (
          <span
            className={`absolute -top-4 -right-16 font-bold text-sm px-4 py-1 rounded-bl-lg rounded-tr-lg ${
              isSelected
                ? "bg-[#36BC9F] text-white"
                : isDarkMode
                ? "bg-[#181a1b] text-[#36BC9F]"
                : "bg-[#f8f4f4] text-[#13556f]"
            }`}
          >
            {discount}
          </span>
      ...
      {/* "Most Popular" footer label */}
      {isPopular && (
        <span
          className={`absolute left-0 right-0 bottom-0 text-xs font-bold px-4 py-1 rounded-b-xl border-t-[1px] ${
