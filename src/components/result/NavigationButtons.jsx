...
const NavigationButtons = ({ result, onPrev, onNext }) => {
  return (
    <div className="w-full max-w-xl px-4 mt-6 mb-8">
      <div className={`flex w-full gap-4 ${result.prevButton ? 'justify-between' : 'justify-end'}`}>
        {result.prevButton && (
          <button
            onClick={onPrev}
            type="button"
            className="text-[#36BC9F] bg-white text-lg font-bold py-2 rounded-lg flex items-center justify-between gap-2 shadow transition hover:opacity-90 w-1/2 cursor-pointer border-2 border-[#36BC9F]"
          >
